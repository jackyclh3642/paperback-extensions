...
const ManHuaGuiSettings_1 = require("./ManHuaGuiSettings");
...
class ManHuaGui {
    ...
    async getSourceMenu() {
        return App.createDUISection({
            ...
            rows: async () => {
                return [
                    (0, ManHuaGuiSettings_1.contentSettings)(this.stateManager),
                    (0, ManHuaGuiSettings_1.resetSettings)(this.stateManager)
                ];
            },
            isHidden: false
        });
    ...
    async getHomePageSections(sectionCallback) {
        ...
        featuredSection.items = (0, ManHuaGuiParser_1.parseFeatured)(await this.getResponse(featuredRequest));
        sectionCallback(featuredSection);
        popularSection.items = (0, ManHuaGuiParser_1.parseHomeSections)(await this.getResponse(popularRequest));
        sectionCallback(popularSection);
        latestSection.items = (0, ManHuaGuiParser_1.parseHomeSections)(await this.getResponse(latestRequest));
        sectionCallback(latestSection);
        newSection.items = (0, ManHuaGuiParser_1.parseHomeSections)(await this.getResponse(newRequest));
        sectionCallback(newSection);
    }
    async getViewMoreItems(homepageSectionId, metadata) {
        ...
        const response = await this.requestManager.schedule(request, 1);
        const $ = this.cheerio.load(response.data);
        const manga = (0, ManHuaGuiParser_1.parseViewMore)($);
        page++;
        if (manga.length < 10)
            page = -1;
    ...
    async getSearchResults(query, metadata) {
        ...
        if (query.includedTags?.length > 0) {
            const queryTags = [];
            for (const tagGroup of tags_json_1.default) {
            ...
            manga = (0, ManHuaGuiParser_1.parseHomeSections)(await this.getResponse(request));
        }
        else {
            ...
            manga = (0, ManHuaGuiParser_1.parseSearchResults)(await this.getResponse(request));
        }
        page++;
        if (manga.length < 10)
            page = -1;
    ...
    async getChapters(mangaId) {
        ...
        const response = await this.requestManager.schedule(request, 1);
        const $ = this.cheerio.load(response.data);
        const showNSFW = await (0, ManHuaGuiSettings_1.getShowNSFW)(this.stateManager);
        const skipExtra = await (0, ManHuaGuiSettings_1.getSkipExtra)(this.stateManager);
        return (0, ManHuaGuiParser_1.parseChapters)($, showNSFW, skipExtra);
    }
...
},{"./ManHuaGuiParser":72,"./ManHuaGuiSettings":73,"./tags.json":74,"@paperback/types":61}],72:[function(require,module,exports){
"use strict";
var __createBinding = (this && this.__createBinding) || (Object.create ? (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    var desc = Object.getOwnPropertyDescriptor(m, k);
...
Object.defineProperty(exports, "__esModule", { value: true });
exports.parseFeatured = exports.parseHomeSections = exports.parseViewMore = exports.parseSearchResults = exports.parseChapterDetails = exports.parseChapters = exports.parseMangaDetails = void 0;
...
const parseMangaDetails = ($, mangaId) => {
    ...
    const tags = [App.createTagSection({ id: 'genre', label: '劇情', tags: arrayTags.map((x) => App.createTag(x)) })];
    return App.createSourceManga({
        ...
        mangaInfo: App.createMangaInfo({
            ...
            author,
            tags,
            desc: (0, entities_1.decodeHTML)(desc)
        })
    });
};
exports.parseMangaDetails = parseMangaDetails;
const parseChapters = ($, showNSFW, skipExtra) => {
    const chapters = [];
    ...
    for (const [index, chapterList] of arrChapters.entries()) {
        ...
        if ((!label.includes("單行本") && (!label.includes("單話"))) && skipExtra)
            continue;
        for (const item of arrButtons) {
            ...
            if (label.includes('單行本'))
                name = name.substring(name.indexOf("卷") + 1);
            name = name.trim();
            if (name == "試看版")
                name = '';
...
const parseChapterDetails = ($, mangaId, id) => {
    ...
    const pages = imgJson.files.map((x) => encodeURI(decodeURI((0, entities_1.decodeHTML)(`https://i.hamreus.com${imgJson.path}${x}?e=${imgJson.sl?.e}&m=${imgJson.sl?.m}`))));
...
},{"entities":69,"opencc-js":70}],73:[function(require,module,exports){
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.resetSettings = exports.contentSettings = exports.getSkipExtra = exports.getShowNSFW = void 0;
const getShowNSFW = async (stateManager) => {
    return await stateManager.retrieve('show_nsfw') ?? false;
};
exports.getShowNSFW = getShowNSFW;
const getSkipExtra = async (stateManager) => {
    return (await stateManager.retrieve('skip_extra') ?? true);
};
exports.getSkipExtra = getSkipExtra;
        ...
        form: App.createDUIForm({
            sections: async () => [
                App.createDUISection({
                    ...
                    rows: async () => {
                        await Promise.all([
                            (0, exports.getShowNSFW)(stateManager),
                            (0, exports.getSkipExtra)(stateManager)
                        ]);
                        return await [
                            App.createDUISwitch({
                                id: 'show_nsfw',
                                ...
                                value: App.createDUIBinding({
                                    get: async () => (0, exports.getShowNSFW)(stateManager),
                                    set: async (value) => { await stateManager.store('show_nsfw', value); }
                                })
                            }),
                            App.createDUISwitch({
                                id: 'skip_extra',
                                ...
                                value: App.createDUIBinding({
                                    get: async () => (0, exports.getSkipExtra)(stateManager),
                                    set: async (value) => { await stateManager.store('skip_extra', value); }
                                })
                            }),
                        ];
                    }
...
module.exports=[
    ...
    },
    {
        ...
        "tags": [
            ...
            { "id": "wuxia", "label": "武俠" },
            { "id": "jizhan", "label": "機戰" },
            { "id": "zhanzheng", "label": "戰爭" },
            { "id": "jingji", "label": "競技" },
            { "id": "tiyu", "label": "體育" },
            { "id": "xiaoyuan", "label": "校園" },
            { "id": "shenghuo", "label": "生活" },
            { "id": "lizhi", "label": "勵志" },
            { "id": "lishi", "label": "歷史" },
            { "id": "weiniang", "label": "偽娘" },
            { "id": "zhainan", "label": "宅男" },
            { "id": "funv", "label": "腐女" },
            { "id": "danmei", "label": "耽美" },
            { "id": "baihe", "label": "百合" },
    ...
    },
    {
        ...
        "tags": [
            { "id": "shaonv", "label": "少女" },
            { "id": "shaonian", "label": "少年" },
            { "id": "qingnian", "label": "青年" },
            { "id": "ertong", "label": "兒童" },
            { "id": "tongyong", "label": "通用" }
        ]
    },
    {
        "id": "year",
